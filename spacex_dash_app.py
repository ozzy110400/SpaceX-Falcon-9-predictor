...
def generate_pie(site):
    if site == 'ALL':
        df = spacex_df.groupby(['Launch Site']).sum()
        df = df[['Class']]
        df = df.reset_index()
        fig = px.pie(df, values=['Class'], names=['Launch Site'], title=['Total Success Launches by Site Launch Site'])
    else:
        df2 = spacex_df[spacex_df['Launch Site']==site]
        df3 = df_2.groupby(['Launch Site','class']).size().reset_index(name='class count')
        
        fig = px.pie(values='class count', names=['class'], title='Success rate of site'+site)
    return fig
